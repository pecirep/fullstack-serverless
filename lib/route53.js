const getStsClient = require("./sts");
...
const addCloudFrontAlias = async (serverless, domains, stsRoleArn) => {
    if (!Array.isArray(domains)) {
        domains = [domains];
    }

    const awsClient = stsRoleArn ? await getStsClient(serverless, stsRoleArn) : serverless.getProvider('aws'),
...
const setupCertificate = async (serverless, domains, stsRoleArn) => {
